...
export default function Tide() {
  ...
  // useGLTF.preload('/model/gltf/thuytrieu.gltf')
  // function Model(props) {
  //   const { scene, animations } = useGLTF('/model/gltf/thuytrieu.gltf')
        ...
        <FullScreen handle={screen}>
          ...
          >
            <Canvas linear camera={{ fov: 35, near: 0.1, far: 20000 }}>
              <Suspense fallback={<Loader />}>
                <Stage contactShadowOpacity={1}>
                  {/* <Model /> */}
                  <Model pause={isPause} events={events} setEvents={setEvents} src='/model/gltf/thuytrieu.gltf' eventsData={(e) => setEvents(e)} reload={isReload}/>
                </Stage>
              </Suspense>
            ...
            <div style={description}>
              <p>
                <strong>{translate('names.model')}:</strong>  {translate('tide')}<br />
                <strong>{translate('names.author')}:</strong> {translate('tide_author')}<br />
                <strong>{translate('names.by')}:</strong> {translate('tide_by')}<br />
              </p>
            </div>
