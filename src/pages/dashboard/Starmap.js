...
export default function Starmap() {
  ...
  // useGLTF.preload('/model/gltf/bandosao.gltf')
  // function Model(props) {
  //   const { scene, animations } = useGLTF('/model/gltf/bandosao.gltf')
        ...
        <FullScreen handle={screen}>
          ...
          >
            <Canvas dpr={[1, 2]}  camera={{ fov: 45, near: 0.1, far: 4000, position: [0, 0, 50] }}>
              <Suspense fallback={<Loader />}>
                <Stage contactShadowOpacity={1}>
                  {/* <Model /> */}
                  <Model pause={isPause} events={events} setEvents={setEvents} src='/model/gltf/bandosao.gltf' eventsData={(e) => setEvents(e)} reload={isReload}/>
                </Stage>
              </Suspense>
            ...
            <div style={description}>
              <p>
                <strong>{translate('names.model')}:</strong>  {translate('starmap')}<br />
                <strong>{translate('names.author')}:</strong> {translate('starmap_author')}<br />
                <strong>{translate('names.by')}:</strong> {translate('starmap_by')}<br />
              </p>
            </div>
